...
@dispatchBy[opcode]
trait Responses extends BasicTypes:
  type Res
  type EventContent
  type ResultContent
  type ErrorContent

  /** Indicates an error processing a request. The body of the message will be
    * an error code ([int]) followed by a [string] error message. Then,
    * depending on the exception, more content may follow. The error codes are
    * defined in Section 9, along with their additional content if any.
    *
    * @param code
    * @param message
    * @param rest
  ...
  /** The result to a query (QUERY, PREPARE, EXECUTE or BATCH messages).
    ...
    */
  @opcode(0x08)
  def RESULT(rest: ResultContent): Res
  ...
  /** An event pushed by the server. A client will only receive events for the
    ...
    * the event type. The valid event types are: \- "TOPOLOGY_CHANGE": events
    * related to change in the cluster topology. Currently, events are sent when
    * new nodes are added to the cluster, and when nodes are removed. The body
    * of the message (after the event type) consists of a [string] and an
    * [inet], corresponding respectively to the type of change ("NEW_NODE" or
    * "REMOVED_NODE") followed by the address of the new/removed node. \-
    ...
    * concerned node. \- "SCHEMA_CHANGE": events related to schema change. After
    * the event type, the rest of the message will be
    * <change_type><target><options> where: \- <change_type> is a [string]
    * representing the type of changed involved. It will be one of "CREATED",
    * "UPDATED" or "DROPPED". \- <target> is a [string] that can be one of
    * "KEYSPACE", "TABLE", "TYPE", "FUNCTION" or "AGGREGATE" and describes what
    * has been modified ("TYPE" stands for modifications related to user types,
    * "FUNCTION" for modifications related to user defined functions,
    * "AGGREGATE" for modifications related to user defined aggregates). \-
    * <options> depends on the preceding <target>: \- If <target> is "KEYSPACE",
    * then <options> will be a single [string] representing the keyspace
    * changed. \- If <target> is "TABLE" or "TYPE", then <options> will be 2
    * [string]: the first one will be the keyspace containing the affected
    * object, and the second one will be the name of said affected object
    * (either the table, user type, function, or aggregate name). \- If <target>
    * is "FUNCTION" or "AGGREGATE", multiple arguments follow: \- [string]
    * keyspace containing the user defined function / aggregate \- [string] the
    * function/aggregate name \- [string list] one string for each argument type
    * (as CQL type)
    *
    * All EVENT messages have a streamId of -1 (Section 2.3).
    *
    * Please note that "NEW_NODE" and "UP" events are sent based on internal
    * Gossip communication and as such may be sent a short delay before the
    * binary protocol server on the newly up node is fully started. Clients are
    * thus advised to wait a short time before trying to connect to the node (1
    * second should be enough), otherwise they may experience a connection
    * refusal at first.
    */
  @opcode(0x0c)
  def EVENT(eventType: string, rest: EventContent): Res
