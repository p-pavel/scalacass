...
trait WriteTypes:
  ...
  /** the write was an unlogged
  batch. No batch log write has been attempted.*/
  def UNLOGGED_BATCH: Res 

/** the write was a counter write
  (batched or not).*/
  def  COUNTER: Res  

   /** the timeout occurred during the
  ...
  def CAS: Res 
   ...
   /** the timeout occured when a write involves
  VIEW update and failure to acqiure local view(MV)
  lock for key within timeout */
  def VIEW: Res
  /**the failure occured when cdc_total_space_in_mb is
